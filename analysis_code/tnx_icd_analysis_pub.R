# Name:     tnx_icd_analysis_pub.R
# Author:   Mike Lape
# Date:     01/17/2023
# Description:
#
#   This program is similar to ukb_icd_analysis_pub.R but instead of 
#   processing UKB data, it handles TNX data testing. It uses data prepared by
#   tnx_icd_data_cleaning_pub.ipynb and tnx_icd_gen_pairs.py to test 
#   for association between a particular disease and a clinical laboratory test
#   for the pathogens of interest.
#   
#   It attempts to match the UKB model for this same ICD code, adjusting for 
#   the same covariates when data is available. Also, when one of the cells in
#   the contingency table drops below the threshold of 5 it switches from 
#   running a logistic regression model (glm) and instead uses and an exact 
#   method, in this case Firth's logistic regression.
#
...
RESULTS_DIR = paste(HOME, '/trinetx/results/tnx_results_',
                    OUT_FILE_DATE,  sep = '')


# Read in all data ####
# Pair directory
pair_dir = glue("{HOME}/trinetx/pair_data")

# Sex specific diseases
sex_spec_dis = read.csv(glue("{HOME}/dicts/sex_specific_codes.txt"), 
                        sep = '\t')

# People with healthy pregnancy in record
health_preg = read.csv(glue("{HOME}/trinetx/procd_data/healthy_pregnancy_data.tsv"),
                       sep = '\t')

# Demo data
cov_dat = data.frame(fread(glue("{HOME}/trinetx/procd_data/procd_covs.tsv", 
                  sep = "\t", data.table = FALSE, showProgress = FALSE)))

# Org test info
org_test_info = read_excel(glue("{HOME}/trinetx/lab_test_data_analysis_latest_manual_review.xlsx"))
org_test_info = org_test_info[((org_test_info$final_type == 'cat') &
                                 (org_test_info$good == 'y')),]
org_lookup = read_excel(glue("{HOME}/trinetx/collapsed_loincs_procd.xlsx"))

# Grab tag to org dict
tag_df = read.csv(glue("{HOME}/trinetx/procd_data/prev_res_to_org_test_lookup.txt"),
                  sep = '\t')
...
lib_ls = c('tidyverse', 'stringr', 'MASS', 'data.table', 'readxl', 'logistf')
...
# Dis-org pair info
pair_info = read.csv(glue("{HOME}/trinetx/pair_data/{curr_icd}/{curr_icd}_summaries.tsv"),
                     sep = '\t')
...
setting_str = paste0("Output Files:\n\tResults:\t\t", RES_FN,
                     "\n\tLog File:\t\t", LOG_FN,
...
for (row_ind in 1:nrow(curr_prev_res)) {
  ...
  curr_dt = as.character(as.POSIXlt(Sys.time()))
  msg_str = paste0("[",curr_dt,"|", curr_icd, "] ", curr_dis_name, "\n\t", 
                    curr_tag, "/", curr_fn_tag, " ", curr_fn_anti, '\n')
  ...
  # Remove any weird duplicate tests
  curr_test_df = unique(curr_test_df)
  
  # Send message about what we are starting to work on.
  curr_dt = as.character(as.POSIXlt(Sys.time()))
  msg_str = paste0("[",curr_dt,"] ","Found ", nrow(curr_test_df), " ", TEST_TYPE, 
                   " tests for ", curr_icd, 
  ...
  if (nrow(curr_test_df) == 0){
    ...
    curr_dt = as.character(as.POSIXlt(Sys.time()))
    msg_str = paste0("\t[",curr_dt," | WARNING]: No tests for ", curr_icd, 
                      ' x ', curr_tag)
  ...
  # Send message about what we are starting to work on.
  curr_dt = as.character(as.POSIXlt(Sys.time()))
  msg_str = paste0("[",curr_dt,"] ", nrow(curr_test_df), " of the ", TEST_TYPE, 
  ...
  for (y in 1:nrow(curr_test_df)) {
    ...
    curr_dt = as.character(as.POSIXlt(Sys.time()))
    msg_str = paste0("\n\n[",curr_dt,"|", curr_icd, "] ", curr_dis_name, "\n\t", 
                    curr_tag, "/",
    ...
    if (file.exists(pair_fn) == FALSE) {
      curr_dt = as.character(as.POSIXlt(Sys.time()))
      msg_str = paste0("\t\t[",curr_dt," | ERROR]: File for ", curr_test_id, 
                        ' [', curr_test_type, ']: ',  curr_test_name, 
                        " does not exist: \n\t\t\t", pair_fn)
    ...
    curr_dt = as.character(as.POSIXlt(Sys.time()))
    msg_str = paste0("\t\t[",curr_dt," | Before - cov filtering] nCase: ", 
                     n_case_bf_cov_chk,  ' | nCon: ',
    ...
    curr_dt = as.character(as.POSIXlt(Sys.time()))
    msg_str = paste0("\t\t[",curr_dt," | After - sex filtering] nCase: ", 
                     length(case_inds), " | nCon: ", length(control_inds))
      ...
      if (('mod_ant' %in% rem_me) | ('mod_dis' %in% rem_me))
      {
        if (DEBUG == TRUE) {
          if ('mod_ant' %in% rem_me) {
            curr_dt = as.character(as.POSIXlt(Sys.time()))
            msg_str = paste0("\t\t[",curr_dt," | WARNING]: ",  curr_test_id, 
                        ' [', curr_test_type, ']: ', curr_test_name, 
                        " mod_ant in remove (lapply) skipping - only 1 level!")
            ...
            msg_str = paste0("\t\t[",curr_dt," | WARNING]: ",  curr_test_id, 
                        ' [', curr_test_type, ']: ', curr_test_name, 
                        " mod_dis in remove (lapply) skipping - only 1 level!")            
    ...
    if (length(unique(dat_df$mod_ant)) == 1 )
    {
      ...
      msg_str = paste0("\t\t[",curr_dt," | WARNING]: ",  curr_test_id, ' [', 
                        curr_test_type, ']: ',  curr_test_name, 
                        " All samples had same value for antibody titer!")
    ...
    if ((nCase < MIN_N_CASES) | (nControl < MIN_N_CONS)) {
      ...
      msg_str = paste0("\t\t[",curr_dt," | WARNING]: ",  curr_test_id, ' [', 
                        curr_test_type, ']: ', curr_test_name, 
                        " Not enough cases or cons right before starting stats")
